    ...
    public abstract class SourceBase : IntellisenseSource
    {
        ...
        public void EditorKeyUp(ICSharpCode.AvalonEdit.TextEditor editor, DepthScanner scanner, System.Windows.Input.KeyEventArgs e)
        {
            ...
            if (e.Key == Key.OemPeriod || KEY == ':')
            {
                ...
                info = ResolveDotPath(words, reso, editor, word);
        ...
        protected BaseTypeInfo ResolveDotPath(string[] words, NameResolver reso, TextEditor editor, string fullText)
        {
            BaseTypeInfo info = null;
            if (words.Length > 0)
            {
            ...
            if (info == null)
                info = reso.GetClassType(editor.Document, editor.TextArea.Caret.Line, fullText);
            //attempt to resolve it from globals
            if (info == null && GetGlobals() != null && GetGlobals().ContainsProperty(fullText))
                info = GetGlobals().GetProperty(fullText, true);
            return info;
        }
    }
}
